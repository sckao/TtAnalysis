...
void JetAnalysis::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
   ...
   int pass = evtSelected->eventSelection( 1, 20., iEvent, "patMET", "patJets" );
   ...
   std::vector<const reco::Candidate*> theJets1 = ttJet->JetSelection( jets, isoMu, 20, JEScale, hJ_Et20, &bTags1, bTagAlgo ) ;
   std::vector<const reco::Candidate*> theJets2 = ttJet->JetSelection( jets, isoMu, 25, JEScale, hJ_Et25, &bTags2, bTagAlgo ) ;
   std::vector<const reco::Candidate*> theJets3 = ttJet->JetSelection( jets, isoMu, 30, JEScale, hJ_Et30, &bTags3, bTagAlgo ) ;
   ...
   if ( pass > -1 ) {
      ...
      if ( theJets1.size() == 4 ) {
         std::vector<const reco::Candidate*> outJets1 = ttJet->SoftJetSelection( jets, isoMu, 20, JEScale, &bTags1, bTagAlgo, hJ_Et20 ) ;
      }
      ttJet->MuonAndJet( theJets2, isoMu[0] , hJ_Et25 );
      ttJet->JetEtSpectrum( theJets2, hJ_Et25 );
      ttJet->JetdRAnalysis( theJets2, hJ_Et25 );
      if ( theJets2.size() == 4 ) { 
         std::vector<const reco::Candidate*> outJets2 = ttJet->SoftJetSelection( jets, isoMu, 25, JEScale, &bTags2, bTagAlgo, hJ_Et25 ) ;
      }
      ttJet->MuonAndJet( theJets3, isoMu[0] , hJ_Et30 );
      ttJet->JetEtSpectrum( theJets3, hJ_Et30 );
      ttJet->JetdRAnalysis( theJets3, hJ_Et30 );
      if ( theJets3.size() == 4 ) { 
         std::vector<const reco::Candidate*> outJets3 = ttJet->SoftJetSelection( jets, isoMu, 30, JEScale, &bTags3, bTagAlgo, hJ_Et30 ) ;
      }
   }
