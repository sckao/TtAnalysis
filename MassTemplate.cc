#include "MassTemplate.h"
#include <iostream.h>

ClassImp(MassTemplate)

MassTemplate::MassTemplate( TString aname, TString channel ){
  ...
  plot2 = hfolder+"/"+channel+"_Norm_"+aname+".gif";
  plot3 = hfolder+"/"+channel+"_CombinedFit_"+aname+".gif";
...
void MassTemplate::showAll( int rbin, int lowBound, int upBound ){

  int nbin = 200./rbin ;
 
  TemplateFitting(rbin, lowBound, upBound );
  TemplateFitting2(rbin, lowBound, upBound );
  ...
  TF1 *func2 = new TF1("func2", MassTemplate::fitBW, lowBound, upBound,3);
...
void MassTemplate::CombinedFitting( int rbin, int lowBound, int upBound ){
  ...
  TH1D*    ttadd = new TH1D("ttadd","", nbin, 0., 400.);

  getFakeData( ttadd, ttstk, rbin);
  ...
  // smooth the background channels
  TF1 *func0 = new TF1("func0",MassTemplate::fitG, lowBound, upBound,3);
  bgadd->Fit( func0, "R0","",110,350);
  double bp0 = func0->GetParameter(0);
  double bp1 = func0->GetParameter(1);

  TF1 *func1 = new TF1("func1",MassTemplate::fitData, lowBound, upBound,6);
  ...
  ttadd->Fit( func1, "R","sames",110,350);
  c3->Update();
  ...
  TF1 *func2 = new TF1("func2",MassTemplate::fitBW, 100, 360, 3 );
  ...
  TF1 *func3 = new TF1("func3",MassTemplate::fitG, 100, 360, 3 );
...
void MassTemplate::TemplateFitting2( int rbin, int lowBound, int upBound ){
...
void MassTemplate::TemplateFitting( int rbin, int lowBound, int upBound ){
...
void MassTemplate::ScaleTemplates( double factor, TH1D* tmp, int B1, int B2 ){
...
void MassTemplate::SmoothTemplate( int type, TH1D* ds, TH1D* ds1, int lowBound, int upBound, int npar ){

  TF1* func0;
  if (type == 0) func0 = new TF1("func0", MassTemplate::fitG  , lowBound, upBound, npar);
  if (type == 1) {
     func0 = new TF1("func0", MassTemplate::fitBW , lowBound, upBound, npar);
...
void MassTemplate::combineBG( TH1D* allbg, int rbin ) {
...
void MassTemplate::getFakeData( TH1D* ttadd, THStack* ttstk,  int rbin ){
...
void MassTemplate::getFakeData( TH1D* ttadd,  int rbin ){
...
void MassTemplate::getData( TH1D* h_data, int rbin ){
...
void MassTemplate::getSignal(TH1D* h_Sg, int rbin ) {
...
void MassTemplate::getBackground(TH1D* h_Bg, int type, int rbin ){
...
Double_t MassTemplate::fitG( Double_t* x, Double_t* par){
...
Double_t MassTemplate::fitGS(Double_t *x, Double_t *par) {
...
Double_t MassTemplate::fitBG(Double_t *x, Double_t *par) {

        return fitG(x,par) + fitTail(x, &par[3] );

}

Double_t MassTemplate::fitBW(Double_t *x, Double_t *par) {
...
Double_t MassTemplate::fitData(Double_t *x, Double_t *par) {
